...
function startSecretNumberGame() {
    let numberOfAttempts = 0; // reset attempts for each new game
    const secretNumber = Math.floor(Math.random() * MAX_NUMBER) + 1;
    let guess = null;

    while (guess !== secretNumber) {
        let guessPrompt = prompt(`Please enter a number between ${MIN_NUMBER} and ${MAX_NUMBER}.`);
        ...
        if (guess > secretNumber) {
            ...
            console.log(`You guessed the number! It took you ${numberOfAttempts} attempts.`);
        }
    }
